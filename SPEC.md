# 書籍プロモーション動画生成システム - 仕様書

**バージョン**: 1.0
**最終更新**: 2025-11-06
**ステータス**: Active

---

## 📋 目次

1. [プロジェクト概要](#プロジェクト概要)
2. [技術選定](#技術選定)
3. [システム要件](#システム要件)
4. [コア機能仕様](#コア機能仕様)
5. [開発原則](#開発原則)
6. [Git運用フロー](#git運用フロー)

---

## プロジェクト概要

### 目的
書籍の表紙画像から、AIを活用してプロモーション動画を自動生成するシステムを構築する。

### ゴール
- Google Veo 3.1で静止画から動画を生成
- Google Cloud TTSで日本語ナレーションを生成
- MoviePyで動画編集（字幕、エフェクト）
- Streamlit UIで誰でも簡単に動画作成

### 対象ユーザー
- 出版社のマーケティング担当者
- 書籍プロモーション担当者
- SNS運用担当者

---

## 技術選定

### コア技術

| 技術 | 用途 | 理由 |
|------|------|------|
| **Google Veo 3.1** | 動画生成 | 静止画から高品質な動画を生成可能 |
| **Google Cloud TTS** | 音声合成 | 日本語の自然な音声合成が可能 |
| **MoviePy** | 動画編集 | Pythonで動画編集が容易 |
| **Streamlit** | UI | 短時間でインタラクティブなUIを構築可能 |
| **Python 3.9+** | 言語 | AI/MLライブラリのエコシステムが充実 |

### 依存ライブラリ
```
google-generativeai  # Veo 3.1 API
google-cloud-texttospeech  # TTS API
moviepy  # 動画編集
streamlit  # UI
Pillow  # 画像処理
python-dotenv  # 環境変数管理
```

---

## システム要件

### 環境要件
- Python 3.9以上
- Google Cloud アカウント
- Google API Key（Veo 3.1用）
- 5GB以上の空きディスク容量

### API要件
- **Veo 3.1 API**: 動画生成
  - モデル: `veo-3.1-generate-preview`
  - 入力: 画像 + プロンプト
  - 出力: MP4動画（最大8秒）

- **Google Cloud TTS**: 音声合成
  - 言語: 日本語（ja-JP）
  - 音声: Wavenet/Neural2
  - 入力: テキスト（最大5000文字）

### パフォーマンス要件
- Veo 3.1動画生成: 3〜5分
- TTS音声生成: 5秒以内
- MoviePy編集: 30秒以内

---

## コア機能仕様

### 1. 動画生成（Veo 3.1）

**入力**:
- 書籍表紙画像（PNG/JPG）
- 生成プロンプト（テキスト）

**処理**:
1. 画像をVeo 3.1 APIに送信
2. プロンプトに基づき動画生成
3. ポーリングで生成完了を待機（最大5分）
4. 生成された動画をダウンロード

**出力**:
- MP4動画ファイル（720p/1080p、4〜8秒）

**エラーハンドリング（Fail-First）**:
```python
# ❌ 禁止: エラー握りつぶし
try:
    video = generate_video(image)
except Exception:
    return None  # NG

# ✅ 推奨: 早期失敗
def generate_video(image_path: str) -> str:
    if not os.path.exists(image_path):
        raise FileNotFoundError(f"Image not found: {image_path}")

    response = veo_api.generate(image_path)
    if response.status != "SUCCESS":
        raise APIError(f"Veo API failed: {response.error}")

    return response.video_path
```

### 2. ナレーション生成（TTS）

**入力**:
- ナレーションテキスト
- 音声タイプ（男性/女性）
- 話速（0.25〜4.0）

**処理**:
1. テキストをGoogle Cloud TTS APIに送信
2. 指定した音声で合成
3. MP3ファイルとして保存

**出力**:
- MP3音声ファイル

**音声一覧**:
- `ja-JP-Wavenet-C`: 男性（低音、重厚）
- `ja-JP-Wavenet-B`: 女性（標準）
- `ja-JP-Neural2-C`: 男性（自然）
- `ja-JP-Neural2-B`: 女性（自然）

### 3. 動画編集（MoviePy）

**機能**:
- テキストオーバーレイ（「続きは本書で」など）
- フェードイン/アウト効果
- 音声トラック合成
- 解像度調整

**処理フロー**:
```
動画ファイル + 音声ファイル
  ↓
MoviePy合成
  ↓
テキストオーバーレイ
  ↓
フェード効果
  ↓
最終動画出力（MP4）
```

### 4. インタラクティブUI（Streamlit）

**画面構成**:
1. **入力セクション**
   - 書籍表紙画像アップロード
   - タイトル入力
   - プロンプト入力
   - ナレーションテキスト入力

2. **パラメータ調整**
   - 動画長さ（4/6/8秒）
   - 音声選択（男性/女性）
   - 話速調整

3. **プレビュー**
   - 生成済み動画プレビュー
   - 音声再生

4. **エクスポート**
   - 最終動画生成
   - ダウンロードボタン

---

## 開発原則

### Fail-First原則（最重要）

**エラーは早期に顕在化させる**

#### 禁止事項
1. **try/exceptでエラーを握りつぶす**
   ```python
   # ❌ 絶対禁止
   try:
       result = risky_operation()
   except Exception:
       return None
   ```

2. **未知の入力を受け入れる**
   ```python
   # ❌ 入力検証なし
   def process(value):
       return value * 2
   ```

3. **Fail-Over（失敗の隠蔽）**
   - エラーを隠してデフォルト値を返す
   - ログだけ出して処理を続ける

#### 推奨事項
1. **早期入力検証**
   ```python
   def process(value: int) -> int:
       if value < 0:
           raise ValueError(f"value must be non-negative, got {value}")
       return value * 2
   ```

2. **明示的なエラー送出**
   ```python
   def generate_video(image_path: str) -> str:
       if not os.path.exists(image_path):
           raise FileNotFoundError(f"Image not found: {image_path}")

       # 処理
       return video_path
   ```

3. **エラーの再送出**
   ```python
   try:
       result = risky_operation()
   except SpecificError as e:
       logger.error(f"Failed: {e}")
       raise  # 必ず再送出
   ```

### コーディング原則

#### 1. DRY (Don't Repeat Yourself)
- 重複コードを避ける
- 共通処理は関数化

#### 2. YAGNI (You Aren't Gonna Need It)
- 将来必要かもしれない機能は作らない
- 今必要なものだけ実装

#### 3. KISS (Keep It Simple, Stupid)
- シンプルで明快な実装
- 過度な抽象化を避ける

#### 4. Boy Scout Rule
- コードを触ったら来た時よりきれいに
- 不要なコード・コメントを削除

#### 5. Dead Code Removal
- 使われないコードは即削除
- 技術的負債を溜めない

### コード品質基準

#### 必須事項
- ✅ 型ヒント（Type Hints）
- ✅ Docstring（Google形式）
- ✅ PEP 8準拠
- ✅ 入力検証（Fail-First）

#### 例
```python
def generate_video(
    image_path: str,
    prompt: str,
    duration: int = 8
) -> str:
    """
    Veo 3.1で動画生成

    Args:
        image_path: 入力画像パス
        prompt: 生成プロンプト
        duration: 動画長さ（秒）

    Returns:
        生成された動画ファイルのパス

    Raises:
        FileNotFoundError: 画像ファイルが存在しない
        ValueError: durationが1〜8秒の範囲外
        APIError: Veo API呼び出し失敗
    """
    # 入力検証（Fail-First）
    if not os.path.exists(image_path):
        raise FileNotFoundError(f"Image not found: {image_path}")

    if not 1 <= duration <= 8:
        raise ValueError(f"duration must be 1-8, got {duration}")

    # 処理
    ...
```

---

## Git運用フロー

### ブランチ戦略（Git Flow）

```
master (main)
  ├─ develop
  └─ feature/*
```

#### ブランチ説明

| ブランチ | 用途 | マージ元 | マージ先 |
|---------|------|---------|---------|
| **master** | プロダクション（常に動作） | develop | - |
| **develop** | 開発統合 | feature/* | master |
| **feature/** | 機能開発 | develop | develop |

### 開発フロー

#### 1. 新機能開発

```bash
# developから派生
git checkout develop
git pull origin develop
git checkout -b feature/123-add-subtitle

# 開発・コミット
git add .
git commit -m "feat: Add subtitle animation"

# リモートにプッシュ
git push -u origin feature/123-add-subtitle

# PR作成（GitHub）
# develop ← feature/123-add-subtitle
```

#### 2. コミットメッセージ規約

**フォーマット**:
```
<type>: <subject>

<body>

<footer>
```

**Type一覧**:
- `feat`: 新機能
- `fix`: バグ修正
- `docs`: ドキュメント変更
- `refactor`: リファクタリング
- `test`: テスト追加
- `chore`: 雑務（ビルド設定等）

**例**:
```
feat: Add subtitle fade-in animation

Implement fade-in effect for subtitles using MoviePy.
- Add fade_duration parameter
- Update UI to control animation speed

Refs: #123
```

#### 3. リリースフロー

```bash
# developが安定したらmasterにマージ
git checkout master
git merge --no-ff develop -m "Release v1.1.0"
git push origin master

# タグ作成
git tag -a v1.1.0 -m "Release version 1.1.0"
git push origin v1.1.0
```

### 禁止事項

1. ❌ **masterへの直接コミット**
2. ❌ **force push**（`git push --force`）
3. ❌ **エラー握りつぶしコミット**
4. ❌ **未テストコードのマージ**

---

## 参考リンク

- [Git運用フロー詳細](docs/git-workflow.md)
- [開発原則（CLAUDE.md）](CLAUDE.md)
- [Google Veo 3.1 Documentation](https://ai.google.dev/veo)
- [Google Cloud TTS Documentation](https://cloud.google.com/text-to-speech/docs)

---

## 変更履歴

| 日付 | バージョン | 変更内容 |
|------|-----------|---------|
| 2025-11-06 | 1.0 | 初版作成 |
